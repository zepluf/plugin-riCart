...
/**
 *
 * Due to the limitation of ZC cart class, we will have to hack it like this be able able to override it
 * @author vunguyen
 *
 */
class ZCCart extends \base
{
    /**
    ...
    function calculate()
    {
        ...
        while (list($products_id,) = each($this->contents)) {
            ...
            if ($product = $db->Execute($product_query)) {
                ...
                $this->face_total += $products_price * $qty;
...
//  echo 'I SEE ' . $this->total . ' vs ' . $this->free_shipping_price . ' items: ' . $this->free_shipping_item. '<br>';
    ...
    function getFaceTotal()
    {
        if ($this->face_total == null)
            $this->calculate();
